...
```jsonc
{
  ...
  "content": "{\"v\":2,\"mintUrl\":\"…/api/coupons/mint\",\"claimUrl\":\"…/api/coupons/claim\",\"refreshUrl\":\"…/api/coupons/refresh\"}"
}
```
...
| content | `refreshUrl` | Optional. Absent means no [refresh](/docs/protocol/refresh) |
...
- `refreshUrl` was added to `v2`, not `v3`. A version bump discards the document whole for every existing reader; an unknown key is ignored harmlessly
- Kind 30078 is addressable: moving hosts is editing one event

Get the CMS pubkey from [`GET /api/coupons/manager`](/docs/api), then sign and [`PUT /api/coupons/discovery`](/docs/api).
